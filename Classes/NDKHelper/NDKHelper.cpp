...
{
    CCLog("into HandleMessage");

    if (methodName == NULL)
        return;
    
    const char *methodNameStr = json_string_value(methodName);
    CCLog("into HandleMessage, methodNameStr: %s",methodNameStr);
...
//            CCDirector::sharedDirector()->getActionManager()->resumeTarget(target);
//            target->runAction(action);
            if (sel) {
                (target->*sel)(NULL,(void*)dataToPass);
            }
